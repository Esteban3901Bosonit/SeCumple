...
{
    protected readonly SeCumpleDbContext _context;

    public BaseRepository(SeCumpleDbContext context)
    {
        _context = context;
    }
    ...
    public async Task<IReadOnlyList<T>> GetAsync(Expression<Func<T, bool>>? predicate,
        Func<IQueryable<T>, IOrderedQueryable<T>>? orderBy, string? includeString, bool disableTracking = true)
    {
    ...
    public async Task<T> GetEntityIncludesAsync(
        ...
        bool disableTracking = true)
    {
        ...
        if (thenIncludes != null)
        {
            ...
            {
                query = thenInclude(query.Include(includes!.Last()));
            }
        }
